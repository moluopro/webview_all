...
* Refactor the package to implement the `webview_flutter` platform interface on Windows.
* Add `WindowsWebViewPlatform`, `WindowsWebViewController`, `WindowsNavigationDelegate`, and `WindowsWebViewWidget`.
* Update the example to use `webview_flutter`'s `WebViewController` and `WebViewWidget` APIs.
